...
export function Card({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border border-white/[0.08] bg-gradient-to-b from-zinc-900/70 to-zinc-900/40 shadow-2xl shadow-black/40 ring-1 ring-white/[0.03] backdrop-blur-xl ${
        glow ? "card-glow" : ""
      } ${className}`}
    >
      {/* top edge highlight */}
  ...
  return (
    <div className="flex items-start justify-between gap-3 border-b border-white/[0.07] px-5 py-4">
      <div className="flex min-w-0 items-start gap-3">
        <span className="mt-1 h-3.5 w-1 shrink-0 rounded-full bg-gradient-to-b from-violet-400 to-cyan-400" />
...
const TONES: Record<Tone, string> = {
  ...
  // `indigo` kept for backward-compat; recoloured to the new violet identity.
...
export function Dot({ tone = "violet" }: { tone?: Tone }) {
  ...
  return (
    <span className="relative inline-flex h-2 w-2">
      <span
        className={`absolute inline-flex h-full w-full rounded-full opacity-60 animate-pulse-soft ${color[tone]}`}
      />
...
export function Button({
  ...
  const styles: Record<string, string> = {
    primary:
      "bg-gradient-to-b from-violet-500 to-violet-600 text-white shadow-lg shadow-violet-950/40 ring-1 ring-inset ring-white/10 hover:from-violet-400 hover:to-violet-500 hover:shadow-violet-900/50 active:translate-y-px",
    secondary:
...
const inputBase =
  "w-full rounded-xl border border-white/10 bg-zinc-950/50 px-3.5 py-2.5 text-sm text-zinc-100 shadow-inner shadow-black/20 transition placeholder:text-zinc-600 focus:border-violet-400/60 focus:outline-none focus:ring-2 focus:ring-violet-500/25";
...
export function Stat({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="rounded-xl border border-white/[0.06] bg-zinc-950/40 px-3.5 py-2.5">
